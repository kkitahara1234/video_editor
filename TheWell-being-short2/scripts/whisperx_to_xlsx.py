...
def split_segment(seg: dict) -> list[dict]:
    """segment の words を行単位に分割して返す（話者境界対応）"""
    words = seg.get('words', [])
    ...
    if not words:
        ...
        if not text:
            return []
        return [{'text': text, 'start': seg['start'], 'end': seg['end'], 'speaker': seg_speaker}]

    # ── 話者ブロック分割: word の speaker を連続塊にグループ化 ──
    # 5word 未満の孤立塊はノイズとして前のブロックに吸収
    MIN_SPEAKER_RUN = 5

    # Step 1: word ごとの speaker リスト
    word_speakers = [w.get('speaker', seg_speaker) for w in words]

    # Step 2: 連続塊化
    runs: list[tuple[str, int, int]] = []  # (speaker, start_wi, end_wi)
    i = 0
    while i < len(word_speakers):
        spk = word_speakers[i]
        j = i
        while j < len(word_speakers) and word_speakers[j] == spk:
            j += 1
        runs.append((spk, i, j))
        i = j

    # Step 3: 短い塊を前のブロックに吸収（ノイズ除去）
    merged_runs: list[tuple[str, int, int]] = [runs[0]]
    for spk, s, e in runs[1:]:
        run_len = e - s
        if run_len < MIN_SPEAKER_RUN:
            # 前のブロックに吸収
            prev = merged_runs[-1]
            merged_runs[-1] = (prev[0], prev[1], e)
        else:
            merged_runs.append((spk, s, e))

    # ── 各話者ブロックを独立に文字数分割 ──
    all_lines = []
    for block_spk, block_start_wi, block_end_wi in merged_runs:
        # このブロックの word → クリーンテキスト + char_to_word マッピング
        clean_chars: list[str] = []
        char_to_word: list[int] = []

        for wi in range(block_start_wi, block_end_wi):
            raw = words[wi].get('word', '')
            for ch in raw:
