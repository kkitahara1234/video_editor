...
## 罠9: Whisper 長 segment の認識漏れ

### 罠9-1: 末尾 word に余り時間が押し込まれ発話が欠落
- 症状: 14.6秒の長い Whisper segment で末尾 word（「を」）が 5.4秒に引き延ばされ、その間の発話が欠落。Remotion で1テロップが7.46秒固定表示
- 原因: Whisper が長い発話区間を1 segment にまとめ、末尾 word に余り時間を押し込む
- 対処: 末尾 word 3秒以上の segment を検出 → 該当区間を ffmpeg 切り出し → Whisper 再実行 → script.json にテロップ追加
- 教訓: 「テロップが切り替わらない」= Remotion バグではなくデータに発話が無い可能性
